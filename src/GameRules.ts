...
export default class GameRules {
  ...
  static isSnakeIsAlive(snake: ISnake): boolean {
    return (
      !GameRules.hasBiteHimself(snake.getHead(), snake.getBody()) &&
      !GameRules.pointOutOfBoundaries(snake.getHead())
    );
  }

  static hasBiteHimself(point: Point, arr: Point[]): boolean {
    return arr.some(
      (i, n, arr) => i.x === point.x && i.y === point.y && n !== arr.length - 1
    );
  }

  static pointOutOfBoundaries(point: Point): boolean {
    return (
      point.x > GameRules.boardWidth ||
      point.x < 0 ||
      point.y > GameRules.boardHeight ||
      point.y < 0
    );
  }
  ...
  static isAppleEaten(snake: ISnake, apple: IApple): boolean {
    ...
    return (
      snakeHeadLocation.x === appleLocation.x &&
      snakeHeadLocation.y === appleLocation.y
    );
  }
